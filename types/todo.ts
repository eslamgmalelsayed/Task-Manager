...
// Helper functions
export const getTodoStatus = (todo: { completed: boolean; due_date: string | null }): TodoStatus => {
    if (todo.completed) return TodoStatus.COMPLETED;
    if (todo.due_date && new Date(todo.due_date) < new Date()) return TodoStatus.PENDING;
